...
// check reported a compiled file and `node --test` then said it could not find
// it, because glob support in `--test` varies by Node version and CI pins 20,
// which has none. The sibling package citadel-workspace-client-ts hit this and
// fixed it; this copy took the checker and kept the glob.
